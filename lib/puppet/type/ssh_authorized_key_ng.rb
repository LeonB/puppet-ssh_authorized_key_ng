module Puppet
  newtype(:ssh_authorized_key_ng) do
    @doc = "Manages SSH authorized keys. Currently only type 2 keys are
    supported.

    **Autorequires:** If Puppet is managing the user account in which this
    SSH key should be installed, the `ssh_authorized_key_mu` resource will autorequire
    that user."
    ...
    newproperty(:user, :array_matching => :all) do
      desc "The user account in which the SSH key should be installed.
        The resource will automatically depend on this user. Multiple values
